...
## Paso 23 - Isolation Forest para detección de anomalías

Isolation Forest se utilizó como técnica no supervisada para detectar viviendas estructuralmente atípicas, sin etiquetas de anomalía. A diferencia de la clasificación, no aprende clases conocidas: aísla observaciones mediante particiones aleatorias; los casos que se separan con pocas particiones reciben mayor atipicidad. Se utilizaron las mismas 36 variables numéricas estructurales del clustering, con imputación por mediana. `Id` y `SalePrice` se excluyeron del ajuste; el precio se incorporó únicamente después para interpretar los resultados. No fue necesario escalar el modelo principal, porque Isolation Forest no depende de distancias euclidianas como K-Means o SVM.

La configuración fue `n_estimators=300`, `contamination="auto"` y `random_state=42`. Se detectaron 70 anomalías de 1460 viviendas (4.79%) y 1390 normales. En los resultados, `anomaly_score` se definió como `-score_samples`, por lo que valores mayores indican mayor atipicidad; también se conservó `decision_function`.

Las variables que más diferenciaron anomalías y normales fueron `GrLivArea`, `TotRmsAbvGrd`, `KitchenAbvGr`, `LotArea`, `1stFlrSF`, `2ndFlrSF`, `TotalBsmtSF`, `LowQualFinSF`, `FullBath` y `BedroomAbvGr`. En promedio, las anomalías presentaron mayor superficie construida (2428.74 frente a 1469.47), mayor área de lote (24131.31 frente a 9831.21), más habitaciones (9.06 frente a 6.39) y más baños completos (2.03 frente a 1.54). Esto describe combinaciones estructurales inusuales, no necesariamente errores.

El `SalePrice` medio fue 265004.71 para anomalías y 176686.77 para viviendas normales; las medianas fueron 212062 y 161500, respectivamente. En la distribución por `PriceGroup`, las anomalías fueron grupo1=6 (8.57%), grupo2=57 (81.43%) y grupo3=7 (10.00%); las normales fueron grupo1=117 (8.42%), grupo2=1271 (91.44%) y grupo3=2 (0.14%). El precio se usó solo para esta interpretación posterior y no se debe interpretar un precio alto como fraude.

La analogía con fraude financiero es conceptual: Isolation Forest puede detectar transacciones raras sin etiquetas, por ejemplo por combinaciones inusuales de monto, horario, ubicación, dispositivo, frecuencia o comercio, y servir como alerta inicial. En housing, una vivienda puede ser atípica por la combinación de tamaño, lote, baños, calidad, antigüedad, garaje o sótano. Una anomalía no equivale a fraude: requiere análisis adicional.
...
## Paso 25 - Reflexión sobre RLHF

RLHF significa Reinforcement Learning from Human Feedback (aprendizaje por refuerzo a partir de retroalimentación humana). Es un enfoque conceptual para orientar el comportamiento de un modelo usando preferencias humanas, en lugar de definir toda la recompensa mediante reglas fijas. No equivale a transferir directamente la inteligencia humana al modelo: las personas proporcionan señales de preferencia que el sistema intenta aproximar y optimizar.

En el Q-learning del Paso 24 el entorno, los estados, las acciones y la recompensa fueron explícitos: el agente recibía -1 por cada paso y +10 al alcanzar la meta, y aprendía una Q-table. En RLHF, los humanos comparan o valoran respuestas y esas preferencias pueden usarse para entrenar un modelo de recompensa. Después se optimiza una política para aumentar la recompensa esperada. La política representa el comportamiento que el agente o modelo usa para elegir acciones; en un sistema generativo, el estado puede simplificarse como prompt y conversación, la acción como token, secuencia o respuesta, y la recompensa como una estimación de calidad o preferencia.

El flujo conceptual clásico es: modelo base → generación de respuestas → evaluación y preferencias humanas → entrenamiento del reward model → optimización de la política → evaluación y alineación. El reward model intenta aproximar qué respuesta sería preferida por los evaluadores, pero puede equivocarse. RLHF no elimina necesariamente el aprendizaje supervisado: un flujo puede incluir preentrenamiento, ajuste supervisado, preferencias humanas, reward model y optimización de política.

Ejemplo: un asistente produce una respuesta A correcta pero poco clara y una respuesta B correcta, clara y segura. Si los evaluadores prefieren B, esa preferencia puede alimentar el aprendizaje de un reward model, que posteriormente orienta la política hacia respuestas similares. Es un ejemplo académico y no una descripción exacta de un producto comercial.

RLHF puede ser útil cuando la recompensa es difícil de expresar con reglas matemáticas, y puede mejorar utilidad, seguimiento de instrucciones y ciertos criterios cualitativos en modelos generativos. Sus riesgos incluyen sesgo de evaluadores, preferencias inconsistentes, coste de anotación, subjetividad, generalización imperfecta, dificultad para representar valores diversos y errores del reward model.

Reward hacking ocurre cuando el agente maximiza la señal de recompensa sin alcanzar realmente el objetivo deseado. Por ejemplo, una métrica mal diseñada puede ser optimizada de forma superficial sin producir la mejor conducta. La alineación tampoco garantiza un comportamiento correcto en todos los casos. En IA generativa, RLHF se relaciona conceptualmente con orientar respuestas para que sean útiles, sigan instrucciones y eviten determinados comportamientos no deseados, sin afirmar detalles propietarios o específicos.

### Respuesta académica y reflexión final
